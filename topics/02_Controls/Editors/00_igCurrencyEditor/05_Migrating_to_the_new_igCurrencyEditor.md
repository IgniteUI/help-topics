...
With the 15.2 release of Ignite UI™ a new set of editor controls was introduced. This includes a reworked `igCurrencyEditor`, with a new design, focused on ease of use and improved UX experience. 

## Topic overview
The aim of this topic is to facilitate the process of migration to the new component. Code snippets are provided to demonstrate the setup of the component both with the previous version and the current one. 
...
---|---|---
animationHideDuration animationShowDuration|Used to set the animation duration when the dropdown list is displayed or hidden.|These options are replaced by new option `dropDownAnimationDuration` that controls the dropdown list animation duration.
button|This option was used to set visibility of the spin and dropdown button.|This option is renamed to `buttonType` to better communicate what it does.
...
dropDownTriggers|-|This option is removed. Now `Alt` + `down-arrow` shows the dropdown.
...
negativeSign|Used to set the character, which is applied to negative numeric values.|Only valid cultural signs are allowed for this option.
nullable|This option was used to control the ability to prevent a null value.|This option is renamed to `allowNullValue` to better communicate what it does.
nullText|Used to set the text which appears in the editor when it has no focus and "value" is null or an empty string.|This option is renamed to `placeHolder` to better communicate what it does.
...
spinOnReadOnly|This was used to override the "readOnly" option and allow changing the "value" of an editor on spin events. |This option is removed. Setting `readOnly` to `true` disables the spin buttons.
...
Option| Description
---|---
disabled|<a name='disabled'></a>Used to set the disabled attribute. Does not allow editing. Disables all the buttons and iteracitons applied. On submit, the current value is not sent into the request: `$('#currencyEditor').igCurrencyEditor({ disabled: true }] })`.
...
---|---|---
addListItem|Used to add the object within an item at the location indicated by the index value.|This method is removed. You should manipulate your data object and rebind the UI component to the updated data for such a scenario.
addListItems|Used to add the objects within the items array at the location indicated by the index value.|This method is removed. You should manipulate your data object and rebind the UI component to the updated data for such a scenario.
...
dropDownElement|Got a reference to the jquery object which is used as a container of the dropdown.|This method is renamed to `dropDownContainter` to better communicate what it does.
dropDownVisible|Set visibility of the dropdown list according to the Boolean value that is passed.|This method is replaced by [showDropDown:](#showDropDown) and [hideDropDown:](#hideDropDown).
...
getSelectedText|Used to get selected text in editor.|This method is removed as it doesn't make sense to have such a method in the currency editor.
getSelection|Used to get left or right edge of selection.|This method is removed as it doesn't make sense to have such a method in the currency editor.
getValueByMode|Used to get the value in editor by dataMode.|This method is not supported.
...
mainElement|Used to get reference to the jquery object which is used as top/outer element of igCurrencyEditor.|This method is renamed to `editorContainter` to better communicate what it does.
...
remove|This was used to remove the editor from its parent element, but keeps the rest of the functionality|This method is not supported. 
...
removeListItemAt|Used to remove an item from list at index.|This method is removed. You should manipulate your data object and rebind the UI component to the updated data for such a scenario.
select|Used to select the text in the editor. Valid parameters: `sel0`, `sel1`, `val`|`sel0` is renamed to `start` and `sel1` is renamed to `end` to prevent exchanging the two parameters. 
setFocus|Used to set focus to the editor with delay.  If parameter was -1, then focus was set without delay.|It now supports only positive values for the delay and 0 is default that signifies that it is immediately given the focus.
spin|Used to increment (positive delta) or decrement (negative delta) the value in the editor according to the parameter.|This method is replaced by two new methods: `spinUp` and `spinDown`. This method works only on the value and not with the dropdown navigation. For the latter you should use [selectListIndexUp:](#selectListIndexUp) or [selectListIndexDown:](#selectListIndexDown) method.
...
validate|Used to trigger validation of the editor and show an error message. It had a single parameter: `noLabel`|The parameter is renamed to `skipErrorMessage`.
...
Method|Description
---|---
currencySymbol|<a name='currencySymbol'></a>Gets ор sets a string that is used as the currency symbol shown with the number in the input. The value provided as a parameter is propagated to the currencySymbol option and thus has the same priority as the option.
showDropDown|<a name='showDropDown'></a>Shows the dropdown list.
hideDropDown|<a name='hideDropDown'></a>Hides the dropdown list.
