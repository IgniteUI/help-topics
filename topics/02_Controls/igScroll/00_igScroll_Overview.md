...
Different environments provide different visualuzations and methods for scrolling. The igScroll supports all of the following types of environments:
...
The way to scroll on mobile is via touch interations. You can swipe in a direction to scroll the content in that direction and the scrollbar will be synced accordingly. 
There is inertia enabled by default, so the content will be scrolled initially fast and then the speed will gradually descrease and the scrolling will stop. This behavior can be further modified via the inertiaDuration option.
...
If you hover the actual scrollbar area the big scrollbars will be displayed and will allow you to further interact with it. 

The following elements of the big scrolbar are interactable:

- the arrow buttons 

	You can click them to scroll with a small increment or click and hold to scroll continuously in a direction.
- the trackpak ( the areas between the thumb and arrow buttons).

	You can click on the trackpad to scroll with a big increment in the related direction.
...
**In HTML:**
 ...
 After initializing an igScroll instance on the div element ( $("#scrollContent").igScroll() ) the resulting DOM will look like this:
...
- [syncedElemsV](%%jQueryApiUrl%%/ui.iggrid#options:syncedElemsV) - Allows setting elements (could be html elements or jQuery elements) that will be linked to the main content container vertically. When the content is scrolled on the Yaxis, the linked elements will scroll up/down.
 
- [syncedElemsH](%%jQueryApiUrl%%/ui.iggrid#options:syncedElemsH) - 
...
Steps 1.is the same as the described in the [Adding igScroll to a Web Page] section above. 

2. Add two scrollable conatiners.
...
**In HTML:**
    ...
    ```
And one positioned on the right:
**In HTML:**
