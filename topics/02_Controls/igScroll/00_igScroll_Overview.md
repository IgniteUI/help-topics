...
```
3. Next initialize the two igScrolls (for each scrollable container) with the option [`modifyDOM`](%%jQueryApiUrl%%/ui.igscroll#options:modifyDOM) set to `false` and set the [`syncedElemsV`](%%jQueryApiUrl%%/ui.igscroll#options:syncedElemsV) and [`syncedElemsH`](%%jQueryApiUrl%%/ui.igscroll#options:syncedElemsH) properties for each of the scrolls as follows:
