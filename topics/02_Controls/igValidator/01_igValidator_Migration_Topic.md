...
checkboxesName| Use the name attribute of checkboxes in order to validate ranges. |This option is removed
element|Gets or sets reference to html element.|This option is removed.
enableTargetErrorCss|Allows user to apply error css to target element.|This option is removed.
...
max, maxLength |Used to determine the maximum length of text or maximum number of selected items.| Those options are replaces by the `lengthRange` property.
min, minLength |Used to determine the minimum length of text or minimum number of selected items.| Those options are replaces by the `lengthRange` property.
...
-	[See all igValidator options](%%jQueryApiUrl%%/ui.ui.igvalidator#options)
-	[See all igNotifier options](%%jQueryApiUrl%%/ui.ui.ignotifier#options)
...
hide| Replaced by the `igNotifier` `hide` method.
isMessageDisplayed| Replaced by the `igNotifier` `isVisible` method.
isValidState| This option is renamed to `isValid`. It takes optional fields.
validate| This option return bool values only and takes optional fields.
...
Event|Changes
---|---
checkValue|This event is renamed to `validating` to better communicate what it does.
validation| This event is renamed to `validated` to better communicate what it does.
