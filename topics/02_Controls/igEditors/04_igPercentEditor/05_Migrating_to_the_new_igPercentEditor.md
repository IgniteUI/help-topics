...
With the 15.2 release of %%ProductName%%™ a new set of editor controls were introduced including a reworked `igPercentEditor`. With a new design focused on simplicity and better UX out-of-the-box some features and their API were revised, removed and new ones added. This topic will cover the differences that will be helpful for developers migrating their applications to the new editors.
...
Option| Description
---|---
[percentSymbol](%%jQueryApiUrl%%/ui.igpercenteditor#options:percentSymbol)|Set custom percent symbol, different from the one in the regional settings.
[allowNullValue](%%jQueryApiUrl%%/ui.igpercenteditor#options:allowNullValue)|<a name='allowNullValue'></a>If that option is false, and editor has no value, then value is set to an empty string.
[disabled](%%jQueryApiUrl%%/ui.igpercenteditor#options:disabled)|<a name='disabled'></a>Use to set the disabled attribute. Does not allow editing. Disables all the buttons and iteracitons applied. On submit the current value is not sent into the request: `$('#percentEditor').igPercentEditor({ disabled: true })`.
[buttonType](%%jQueryApiUrl%%/ui.igpercenteditor#options:buttonType)|<a name='buttonType'></a>Sets gets visibility of spin, clear and drop-down button.
[listItemHoverDuration](%%jQueryApiUrl%%/ui.igpercenteditor#options:listItemHoverDuration)|<a name='listItemHoverDuration'></a>Use to set the hover/unhover animation duration of items inside the dropdown list: `$('#percentEditor').igPercentEditor({ listItems: ["A", "B", "C", "D"], listItemHoverDuration: 400 })`.
[dropDownAttachedToBody](%%jQueryApiUrl%%/ui.igpercenteditor#options:dropDownAttachedToBody)|<a name='dropDownAttachedToBody'></a>Gets sets location of drop-down list.
[dropDownAnimationDuration](%%jQueryApiUrl%%/ui.igpercenteditor#options:dropDownAnimationDuration)|<a name='dropDownAnimationDuration'></a>Gets sets show/hide drop-down list animation duration in milliseconds.
[visibleItemsCount](%%jQueryApiUrl%%/ui.igpercenteditor#options:visibleItemsCount)|<a name='visibleItemsCount'></a>Use to set how many items should be shown at once: `$('#percentEditor').igPercentEditor({ listItems: ["A", "B", "C", "D"], visibleItemsCount: 2 })`.
[isLimitedToListValues](%%jQueryApiUrl%%/ui.igpercenteditor#options:isLimitedToListValues)|<a name='isLimitedToListValues'></a>Use to set the ability to allow values only set into the list items. Enabling this also causes spin actions to cycle through list items instead: `$('#percentEditor').igPercentEditor({ listItems: ["A", "B", "C", "D"], isLimitedToListValues: true })`.
...
[spinUp](%%jQueryApiUrl%%/ui.igpercenteditor#methods:spinUp)|<a name='spinUp'></a>Increments value in editor according to the parameter or selects the previous item from the drop-down list if `isLimitedToListValues` is enabled.
[spinDown](%%jQueryApiUrl%%/ui.igpercenteditor#methods:spinDown)|<a name='spinDown'></a>Decrements value in editor according to the parameter selects the next item from the drop-down list if `isLimitedToListValues` is enabled.
