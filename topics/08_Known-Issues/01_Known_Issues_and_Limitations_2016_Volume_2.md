...
![](../images/images/negative.png)
		...
		<tr>
            <td>
Spinning up when the maximum value is reached and spinWrapAround option is true is not possible if  maxValue is not set with dataMode being float, long or double
			</td>

            <td>
When you use the [`minValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:minValue)option in the igEditors without setting the [`maxValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:maxValue) option, you should keep in mind that the [`maxValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:maxValue) for the editor will be set to the maximum number for the [`dataMode`](%%jQueryApiUrl%%/ui.ignumericeditor#options:dataMode) that is selected. When you also have spinWrapAround set to true and you try to spin up u when the maximum value is already reached, trying to set the value to the opposite side of the limit, the value will not change. This is in case the  [`dataMode`](%%jQueryApiUrl%%/ui.ignumericeditor#options:dataMode) option is set to any of the following values: float, long or double. 
