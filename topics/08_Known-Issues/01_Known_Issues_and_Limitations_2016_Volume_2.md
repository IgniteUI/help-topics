...
This summarizes the known issues and limitations of the Ignite UI™ 2016 Volume 2 release. Information about previous releases can be found [here](Known-Issues-Revision-History.html).
...
The following table summarizes the known issues and limitations of the Ignite UI 2016 Volume 2 release. Detailed explanations of known issues and the possible workarounds are provided in the known issues topics for each control.
<a id="legend"></a>
...
![](../images/images/negative.png)
		...
		<tr>
            <td>
Spinning up using the spin buttons when the maximum value is reached and spinWrapAround option is true is not possible if  maxValue is not set with dataMode being float, long or double
			</td>

            <td>
When you use the [`minValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:minValue)option in the igEditors without setting the [`maxValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:maxValue) option, you should keep in mind that the [`maxValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:maxValue) for the editor will be set to the maximum number for the [`dataMode`](%%jQueryApiUrl%%/ui.ignumericeditor#options:dataMode) that is selected. When you also have spinWrapAround set to true and you try to spin up using the spin buttons when the maximum value is already reached, trying to set the value to the opposite side of the limit, the value will not change. This is in case the  [`dataMode`](%%jQueryApiUrl%%/ui.ignumericeditor#options:dataMode) option is set to any of the following values: float, long or double. 
...
To bypass this limitation, you can set the [`maxValue`](%%jQueryApiUrl%%/ui.ignumericeditor#options:maxValue) to a number that is not represented in scientific notation by JavaScript or you can set the  [`scientificFormat`](%%jQueryApiUrl%%/ui.ignumericeditor#options:scientificFormat) option in the igEditor.  
            </td>
